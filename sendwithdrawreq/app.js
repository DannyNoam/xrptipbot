...
const api       = new RippleAPI({ server: 'wss://ws01.casinocoin.org:4443' }) // Public rippled server
...
var twilioClient = new twilio(configFile.twilio.accountSid, configFile.twilio.authToken);
...
var _processTransaction = function () {
  ...
  api.getAccountInfo(payFrom).then(function(accInfo){
    ...
    var transaction = {
        ...
        "Amount" : (cscAmount*1000*1000*100).toFixed(0)+"",
...
// Connected
  api.getServerInfo().then(function (server) {
    fee = ((parseFloat(server.validatedLedger.baseFeeCSC) * 1000 * 1000).toFixed(0))*1000;

...
if(!empty($o_postdata) && !empty($o_postdata['name'])){
    try {
        $query = $db->prepare('SELECT * FROM user WHERE userid = :userid AND network = :network');
        $query->bindParam(':userid', $o_postdata['userid']);
        $query->bindParam(':network', $o_postdata['type']);
        $query->execute();
        $row = (object) $query->fetch(PDO::FETCH_ASSOC);

        if(!empty($row->userid) && !empty($row->balance) && $row->balance > 0 && (float) $o_postdata['amount'] <= (float) $row->balance){
            $query = $db->prepare('
            ...
            $query->bindValue('user', @$o_postdata['userid']);
            ...
            if(!empty($insertId)){
                ...
                $query = $db->prepare('UPDATE `user` SET `balance` = `balance` - :withdraw_amount WHERE `userid` = :userid AND `network` = :network LIMIT 1');
                $query->bindValue(':withdraw_amount', $amount);
                $query->bindValue(':userid', $o_postdata['userid']);
                $query->bindValue(':network', $o_postdata['type']);

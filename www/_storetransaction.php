...
$doNotProcess = false;
$route = '';
$originalPostdata = json_decode(file_get_contents('php://input'));

if(!empty($originalPostdata)){

    $o_postdata = (object) $originalPostdata;
    $insertId = 0;
    ...
    try {
        if (empty($o_postdata->type) || $o_postdata->type == 'Payment') {
            ...
            $query->bindParam(':xrp', $o_postdata->csc);
            ...
            if (!empty($o_postdata->type)) {
                if ($o_postdata->type == 'EscrowCreate') {
                    ...
                    ');
                    $amount = empty($o_postdata->fullTx->Amount) ? 0 : (float) $o_postdata->fullTx->Amount / 1000 / 1000 / 100;

...
try {
    ...
    if(!empty($msgs)){
        foreach($msgs as $m){
                    ...
                    if(strtolower($m['parent_author']) == strtolower($m['from_user'])){
                        ...
                        if(!empty($match[1][0])) {
                            ...
                            if ((float) $amount > $__MAX_TIP_AMOUNT) {
                                $msg = '@'.$m['from_user'] . " Unfortunately, the maximum tip amount is " . $__MAX_TIP_AMOUNT . " CSC. Please feel free to tip @" . $m['parent_author'] . " a smaller amount.";
                            } else if(substr_count($amount, '.') > 1) {
                                ...
                                if(empty($m['_from_user_name'])){
                                    $msg = '@'.$m['from_user'] . " You cannot send tips until you deposit some ".'$CSC'." at https://www.csctipbot.com/deposit ...";
                                }else{
                                    $twitterUserId = $twitter_call('/users/show', 'GET', ['screen_name' => $m['parent_author']])->id;
                                    $fromTwitterUserId = $twitter_call('/users/show', 'GET', ['screen_name' => $m['from_user']])->id;

                                    if(empty($m['_to_user_name'])){
                                        // Create TO user
                                        $query = $db->prepare('INSERT IGNORE INTO user (username, userid, create_reason, network) VALUES (:username, :userid, "TIPPED", "twitter")');
                                        $query->bindValue(':username', $m['parent_author']);
                                        $query->bindValue(':userid', $twitterUserId);
                                        $query->execute();
                                    }

                                    if($m['_from_user_balance'] < $amount){
                                        $msg = '@'.$m['from_user'].' Unfortunately, your balance is insufficient to tip other users. Please deposit some $CSC at https://www.csctipbot.com/deposit first and tip @'.$m['parent_author'].' again.';
                                    }else{
                                        if(strtolower($m['parent_author']) == 'csctipbot'){
                                            ...
                                            $msg = '@' . $m['parent_author'] . ' You have received a tip: ' . $amount . ' $CSC' . $usdamount . ' from @' . $m['from_user'] . '. Withdraw your tip from ';
                                            // if(empty($m['_to_user_name'])){
                                        ...
                                        $query->bindValue(':from', $fromTwitterUserId);
                                        $query->bindValue(':to', $twitterUserId);
            ...
            if(strtolower($m['parent_author']) == 'csctipbot' && strtolower($m['to_user']) == 'csctipbot'){
            ...
            echo `cd /data; php send_twitter_reaction.php '$to_post' '$msg_escaped'`;
            sleep(2);
        }
    }
}

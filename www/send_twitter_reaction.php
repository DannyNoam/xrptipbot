...
echo "Sending Twitter reply...\n";
echo $text;

if(!empty($original_text)){
    $tipboturl = ' https://www.csctipbot.com';
    if (substr_count($text, 'csctipbot.com') > 0) {
        $tipboturl = '';
    }

    $postdata = [
        'status' => $text." $tipboturl 🎉 #CasinoCoin",
    ];
    if (!empty($parent_id)) {
...
$callbackurl = '';
if(!empty($post->id)){
    $callbackurl = "https://twitter.com/csctipbot/status/" . @$post->id;
